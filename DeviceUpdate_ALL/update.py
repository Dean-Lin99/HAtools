...
def guess_content_type(filename: str) -> str:
    ...
    if name.endswith(".db") or name.endswith(".sqlite") or name.endswith(".db3"):
...
class DownloadHandler(BaseHTTPRequestHandler):
    ...
    def _get_meta_by_route(self, route: str):
        with DownloadHandler.lock:
            return DownloadHandler.registry.get(route)
    ...
    def _resolved_ctype(self, route: str, meta: dict) -> str:
        return DownloadHandler.ctype_override.get(route, "") or meta["content_type"]
...
class UpdateWorker(QThread):
    ...
    @staticmethod
    def _action_candidates_for_file(filename: str):
        ext = os.path.splitext(filename)[1].lower()
        fw_actions = ["sw_upgrade", "upgrade", "sw-upgrade"]
        db_actions = ["iptable_sync", "iptables_sync", "db_sync", "db_upgrade", "db_update"]
        if ext in (".db", ".sqlite", ".db3"):
            # .db 類：優先同步/DB 動作，再 fallback 到韌體動作（以防廠商端寫死只吃 upgrade）
            return db_actions + fw_actions
        else:
            # 韌體類：先韌體動作，再 fallback 到 DB 動作
            return fw_actions + db_actions
    ...
    def run(self):
        ...
        def do_one(dev):
            ...
            # 依檔名決定 action 嘗試順序
            action_candidates = self._action_candidates_for_file(filename)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("設備軟體/房號表更新工具")
        ...
        # 檔案清單：每筆 {path, filename, route, size, md5_hex, keyword, ctype_override}
    ...
    def _build_ui(self):
        ...
        gb_fw = QGroupBox("檔案清單（軟體/IPTABLE）")
        vfw = QVBoxLayout(gb_fw)
        btns = QHBoxLayout()
        btn_add_fw = QPushButton("加入檔案")
    ...
    def _rebuild_server_registry(self):
        ...
        if bad:
            QMessageBox.warning(self, "無法加入的檔案", "以下檔案無法載入，已略過：\n\n" + "\n".join(bad))

    def on_add_fw(self):
        path, _ = QFileDialog.getOpenFileName(self, "選擇檔案", "", "檔案 (*.pkg *.apk *.bin *.zip *.db *.sqlite *.db3);;所有檔案 (*.*)")
        if not path:
            return
    ...
    def start_update(self):
        ...
        if not self.fw_items:
            QMessageBox.warning(self, "提醒", "請先於『檔案清單』加入至少一個檔案。")
            return
        ...
        if not worker_fws:
            QMessageBox.warning(self, "提醒", "沒有可用的檔案（皆無法載入）。")
            return
        ...
        # 策略：固定 vendor path + opaque verify
